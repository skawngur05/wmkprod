...
for (const line of dataLines) {
    try {
        ...
        // Build comprehensive notes field with all available information
        let notesField = '';
        const notesParts = [];
        ...
        const sqlValues = `('${escapeSqlString(nameField)}', '${escapeSqlString(phone.trim())}', '${escapeSqlString(email.trim())}', 'Commercial', 'Commercial', NULL, '', 'New', '${escapeSqlString(notesField)}', '2025-10-02', NULL)`;
...
const sqlHeader = `-- Import all commercial leads from CSV data
...
// Write to file
const outputPath = path.join(__dirname, 'migrations', 'commercial_leads_complete.sql');
fs.writeFileSync(outputPath, completeSql, 'utf-8');

console.log(`\nGenerated complete SQL file: ${outputPath}`);
console.log(`Total INSERT statements: ${sqlStatements.length}`);
console.log('\nFirst 3 generated statements:');
sqlStatements.slice(0, 3).forEach((stmt, index) => {
    console.log(`${index + 1}. ${stmt}`);
});

console.log('\nLast 3 generated statements:');
sqlStatements.slice(-3).forEach((stmt, index) => {
    console.log(`${sqlStatements.length - 2 + index}. ${stmt}`);
});
